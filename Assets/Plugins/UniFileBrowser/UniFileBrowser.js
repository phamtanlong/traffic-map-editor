...
private function DrawFileWindow () {
	...
	if (fileType != FileType.Folder) {
		if (fileType == FileType.Save && preventIllegalCharInput) {
	...
	if (fileType == FileType.Open) {
		GUI.Label (Rect(fileWindowInset, textfieldRect.y, 90, textfieldRect.height), "Open file:");
	}
	...
	var buttonRect = Rect(fileAreaRect.x, fileAreaRect.y, 9999, linePixelHeight);	
	for (i = 0; i < fileDisplayList.Length; i++) {
		if (GUI.Button (buttonRect, fileDisplayList[i], ((multi && multiFileList.Contains (i)) || (!multi && i == selectedFileNumber))? scrollViewStyleSelected : scrollViewStyle) && frameDone) {
			...
			if (multi) {
				...
				fileName = (multiFileList.Count == 1 && i >= dirList.Count && i - dirList.Count < fileList.Count)?
					fileDisplayList[multiFileList[0]].text : "";
			}
		}
		...
		if (multi) {
			...
			if (fileType == FileType.Save) {
				...
				fileName = (selectedFileNumber >= dirList.Count && selectedFileNumber - dirList.Count < fileList.Count)?
							fileList[selectedFileNumber - dirList.Count] : ""; // No file name if directory is selected
			}
		}
	...
	if (Input.GetMouseButtonDown(0) && !clicked) {
		if (Time.time - clickTimer < doubleClickTime && fileAreaRect.Contains (mousePos) && frameDone) {
			doubleClicked = true;
		}
...
#else
	if (Event.current.clickCount == 2 && fileAreaRect.Contains (mousePos) && frameDone) {
		doubleClicked = true;
	}
#endif
	...
	for (var touch in Input.touches) {
		if (touch.tapCount == 2 && fileAreaRect.Contains (mousePos) && frameDone) {
			SelectFile();
			WaitForFrame();
			messageWindowDelay = true;
			break;
	...
	if (!limitToInitialFolder) {
		if (pathList.Length > 0 && Popup.List (popupRect, showPopup, selectedPath, pathList[0], pathList, popupButtonStyle, popupBoxStyle, popupListStyle)) {
			if (selectedPath.i > 0 && !limitToInitialFolder) {
...
private function MessageWindow () {
	GUI.Label (Rect(fileWindowInset, windowOffset + 10, messageWindowRect.width - fileWindowInset*2,
...
private function ShowFileWindow () {
	...
	fileBoxRect = Rect(fileWindowInset, textfieldRect.y + textfieldRect.height + (fileType == FileType.Folder? 0 : 15), 0, 0);
	fileAreaRect = fileBoxRect;
	fileAreaRect.x += xIndent;
	fileAreaRect.y += yIndent;
	bottomAreaSpace = fileWindowInset + 15 + buttonSize.y;
	scrollbarWidth = guiSkin.GetStyle ("verticalscrollbar").fixedWidth;
	...
	if (!_allowMultiSelect) {
		...
		for (var i = 0; i < files.Count; i++) {
			if (File.Exists (files[i])) {
				var thisFileName = files[i].Substring (files[i].LastIndexOf (pathChar) + 1);
...
private function GetMultiFileNames () : List.<String> {
	...
	for (var i = 0; i < multiFileList.Count; i++) {
		if (multiFileList[i] >= dirList.Count) {
			files.Add (filePath + fileList[multiFileList[i] - dirList.Count]);
		}
	}
	return files;
}
